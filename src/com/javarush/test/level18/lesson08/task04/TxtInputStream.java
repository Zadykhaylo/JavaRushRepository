...
public class TxtInputStream extends FileInputStream {
    ...
    public TxtInputStream(String fileName) throws FileNotFoundException, UnsupportedFileNameException
    {
        ...
        s.append(name[name.length-4]);
        s.append(name[name.length-3]);
        s.append(name[name.length-2]);
        s.append(name[name.length-1]);
        String ss = s.toString();
        if (!ss.equals(".txt")) throw new UnsupportedFileNameException();
